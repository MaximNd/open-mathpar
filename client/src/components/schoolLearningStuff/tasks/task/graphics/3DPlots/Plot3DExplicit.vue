<template>
  <div ref="plot3d-explicit-container" class="plot3d-explicit-container">
...
export default {
  ...
  methods: {
    ...
    initData() {
      this.$store.dispatch('plot3dExplicit', { sectionId: this.sectionId, task: this.task })
        .then(({ body }) => {
          const xMin = body[0][0];
          const xMax = body[0][1];
          const yMin = body[0][2];
  ...
  mounted() {
    ...
    canvas.classList.add('plot3d-explicit-canvas');
    canvas.width = this.canvasWidth;
    canvas.height = this.canvasHeight;
    this.$refs['plot3d-explicit-container'].appendChild(canvas);
  },
  created() {
    this.init();
    this.initData();
...
<style>
  .plot3d-explicit-container {
    width: 100%;
    height: 100%;
  }
  .plot3d-explicit-canvas {
    z-index: 2;
