<template>
  <v-card>
    ...
    <v-form @submit.prevent="createTeacher">
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn type="submit" color="success" button @click.native="createTeacherDialog = false">Save</v-btn>
        <v-spacer></v-spacer>
      </v-card-actions>
    </v-form>
  </v-card>
...
export default {
  ...
  methods: {
    ...
    createTeacher() {
      this.$store.dispatch('createTeacherU', { user: this.user, teacher: this.teacher })
        .then(() => {
          ...
          while(this.teacher.timetable.length) {
            this.teacher.timetable.pop();
          }
